...
class ManageInstrumentsSettings extends BaseSettingsPage
{
    protected static ?string $navigationIcon = 'heroicon-o-wrench-screwdriver';

    protected static string $settings = InstrumentsSettings::class;
    
    protected static ?string $navigationLabel = 'Nastavitve Meril';
    ...
    protected static ?string $slug = 'nastavitve/merila';
            ...
            Action::make('testEmail')
                ...
                ->action(function () {
                    try {
                        ...
                        // Kliči command za pošiljanje
                        Artisan::call('instruments:send-reminders');
                        $output = Artisan::output();
                        
                        Notification::make()
                            ->title('Uspešno poslano!')
                            ->body('Testno obvestilo je bilo poslano. Preverite email: ' . $instrumentsSettings->notification_recipients)
                            ->success()
                            ->duration(5000)
                            ->send();
