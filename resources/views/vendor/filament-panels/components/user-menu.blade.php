@php
    ...
    // Fallback za profile URL – če auth.profile ni definiran
    $profileUrl = $profileItemUrl ?? (function () {
        try {
            return filament()->getProfileUrl();
        } catch (\Throwable $e) {
            $panelId = filament()->getId();
            $authProfile = "filament.{$panelId}.auth.profile";
            $pagesProfile = "filament.{$panelId}.pages.profile";
            return \Illuminate\Support\Facades\Route::has($pagesProfile)
                ? route($pagesProfile)
                : url(filament()->getPath() . '/profile');
        }
    })();
